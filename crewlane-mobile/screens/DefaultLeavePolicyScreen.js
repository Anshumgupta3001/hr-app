...
export default function DefaultLeavePolicyScreen({ navigation }) {
    ...
    useCallback(() => {
      ...
      async function load() {
        ...
        if (current.role !== 'superadmin') {
          navigation.replace('Dashboard', { companyId: current.companyId });
          return;
        }
        setUser(current);
        if (!loaded) {
      ...
      >
        ...
        >
          <OutlinedCard style={styles.cardWrap}>
            <BackButton />
            <Text style={styles.heading}>Default Leave Policy</Text>
