...
export const documentService = {
  ...
  async uploadDocument({ companyId, employeeId, documentType, file, label = '' }) {
    ...
    const localFileRef = await localFileStore.saveFile(file);
    return {
      id: `doc-${uuidv4()}`,
      companyId,
      employeeId,
  ...
  async deleteStoredFile(record) {
    ...
    try {
      if (record.storageType === 'local') {
        if (record.localFileRef) await localFileStore.deleteFile(record.localFileRef);
      }
  ...
  async deleteByEmployee() {
    // No server-side call needed here — deletion happens per document via
    // removeDocument.
  },
};
